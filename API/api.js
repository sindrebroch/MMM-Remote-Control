...
module.exports = {
   ...
   */
  getApiKey: function () {
    let thisConfig = this.configOnHd.modules.find(
      (x) => x.module === "MMM-Remote-Control"
    );
    if (typeof "thisConfig" !== "undefined" && "config" in thisConfig) {
      if ("apiKey" in thisConfig.config && thisConfig.config.apiKey !== "") {
        this.apiKey = thisConfig.config.apiKey;
      } else {
        this.apiKey = undefined;
      }
      if (
        "secureEndpoints" in thisConfig.config &&
  ...
  getExternalApiByGuessing: function () {
    ...
    this.configOnHd.modules
      .filter((mod) => skippedModules.indexOf(mod.module) === -1)
      .forEach((mod) => {
        try {
          let modActions = getActions(Module.notificationHandler[mod.module]);

          if (modActions.length > 0) {
            let pathGuess = mod.module
              .replace(/MMM-/g, "")
              .replace(/-/g, "")
              .toLowerCase();
            ...
            if (pathGuess in this.externalApiRoutes) {
              this.externalApiRoutes[pathGuess].actions = Object.assign(
                {},
                actionsGuess,
                this.externalApiRoutes[pathGuess].actions
              );
            } else {
          ...
          console.warn(
            `getExternalApiByGuessing failed for ${mod.module}: ${err.message}`
          );
        }
      });
  ...
  createApiRoutes: function () {
    ...
    this.expressRouter
      .route(["/test", "/"]) // Test without apiKey
      .get((req, res) => {
    ...
    this.expressRouter.use((req, res, next) => {
      if (typeof this.apiKey !== "undefined") {
        if (
          ...
          req.headers.authorization.search(/(apikey|bearer)/gi) === -1
        ) {
    ...
    this.expressRouter.route("/classes/:value").get((req, res) => {
      var classes =
        self.getConfig().modules.find((m) => m.module === "MMM-Remote-Control")
          .config || {};
      const val = decodeURIComponent(req.params.value);
      if (classes.classes && classes.classes[val]) {
        self.executeQuery(
          { action: "MANAGE_CLASSES", payload: { classes: req.params.value } },
          res
        );
      } else {
        res.status(400).json({
    ...
    this.expressRouter.route("/command/:value").get((req, res) => {
      if (!this.apiKey && this.secureEndpoints)
        return res.status(403).json({
          success: false,
          message:
    ...
    this.expressRouter.route("/userpresence/:value?").get((req, res) => {
      if (req.params.value) {
        if (req.params.value === "true" || req.params.value === "false") {
          self.executeQuery(
            { action: "USER_PRESENCE", value: req.params.value === "true" },
            res
          );
        } else {
          res.status(400).json({
    ...
    this.expressRouter.route("/update/:moduleName?").get((req, res) => {
      ...
      switch (req.params.moduleName) {
        case "mm":
        case "MM":
          self.answerGet({ data: "mmUpdateAvailable" }, res);
          break;
        case "rc":
        case "RC":
          this.updateModule("MMM-Remote-Control", res);
          break;
        default:
    ...
    this.expressRouter
      ...
      .get((req, res) => {
        res
          .status(400)
          .json({ success: false, message: "Invalid method, use POST" });
      })
      .post((req, res) => {
        if (!this.apiKey && this.secureEndpoints)
          return res.status(403).json({
    ...
    this.expressRouter
      ...
      .get((req, res) => {
        ...
        this.executeQuery(
          this.checkDelay({ action: `MONITOR${actionName}` }, req),
          res
        );
      })
      .post((req, res) => {
        ...
        if (typeof req.body !== "undefined" && "monitor" in req.body) {
          if (
            ["OFF", "ON", "TOGGLE"].includes(req.body.monitor.toUpperCase())
          ) {
    ...
    this.expressRouter.route("/brightness/:setting(\\d+)").get((req, res) => {
      this.executeQuery(
        { action: `BRIGHTNESS`, value: req.params.setting },
        res
      );
    });

    this.expressRouter.route("/timers").get((req, res) => {
      this.sendResponse(res, undefined, this.delayedQueryTimers);
    });
  ...
  mergeData: function () {
    ...
    modules.forEach((mod) => {
      if (extApiRoutes[mod.name] === undefined) {
        ...
        query.data.push(
          Object.assign({}, mod, { actions: extApiRoutes[mod.name].actions })
        );
      }
    });
  ...
  answerModuleApi: function (req, res) {
    ...
    if (
      ["SHOW", "HIDE", "FORCE", "TOGGLE", "DEFAULTS"].indexOf(action) !== -1
    ) {
      // /api/modules part of the code
  ...
  updateModuleApiMenu: function () {
    if (!this.thisConfig.showModuleApiMenu) {
      return;
    }
      ...
      Object.keys(this.externalApiRoutes[r].actions).forEach((a) => {
        ...
        if ("prettyName" in this.externalApiRoutes[r].actions[a]) {
          item.text = this.translate(
            this.externalApiRoutes[r].actions[a].prettyName
          );
        } else {
          item.text = this.translate(
            this.externalApiRoutes[r].actions[a].notification
          )
            .toLowerCase()
